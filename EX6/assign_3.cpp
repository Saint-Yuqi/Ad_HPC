...
int main(int argc, char *argv[]) {
    ...
    if (!io) {
        std::cerr << "Unable to open tipsy file " 
                  << argv[1] << std::endl;
        return errno;
    }
    ...
    // Careful. For odd nGrid, 2*k_nz != nGrid + 2
    float* data = new (std::align_val_t(64)) float[n_floats]; // 512-bit alignment
    Array<float,3> raw_grid(data, shape(nGrid,nGrid,2 * k_nz), 
                            deleteDataWhenDone);
    ...
    // This creates four different versions of "assign mass", 
    // one for each order
