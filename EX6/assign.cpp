...
void assign_mass(Array<float,3> &grid, Array<float,2> &R,Array<float,1> &M) {
    auto nGrid = grid.rows(); // total number of cells along each axis
...
int main(int argc, char *argv[]) {
    ...
    // The second parameter will have the grid size, and optionally
    // the number of bins to use, e.g.,
    //   100      Use a grid size of 100, and use 50 bins
    //   100/40   Grid size is 100 and use 40 bins
    //   100/L35  Grid size is 100, and use 35 logarithmic bins
    ...
    auto rate = (sizeof(tipsy::header) + N*sizeof(tipsy::dark)) 
                / dt.count() / 1024 / 1024;
    std::cerr << "File reading took " 
              << std::setw(9) << dt.count() 
              << " seconds (" << rate << " MB/s)." << std::endl;

    // Create Mass Assignment Grid
    t0 = hrc::now();
    auto k_nz = nGrid/2 + 1;
    auto n_floats = size_t(1) * nGrid * nGrid * 2*k_nz; // 2*k_nz is the "padded" dimension
    ...
    Array<std::complex<float>,3> kgrid(reinterpret_cast<std::complex<float>*>(data),
    ...
    std::cerr << "Total mass assigned is " 
              << std::setw(9) << blitz::sum(grid) << std::endl;

    // Calculate delta = (rho / rho_bar) - 1
    ...
    // **Initialize FFTW threads (OpenMP)**
    if (fftwf_init_threads() == 0) {
        std::cerr << "Error initializing FFTW threads." << std::endl;
        return 1;
    }
    
    // **Set number of threads** 
    // (You can hardcode a value like 4, or use omp_get_max_threads().)
    int nThreads = omp_get_max_threads(); 
    fftwf_plan_with_nthreads(nThreads);
    std::cerr << "Using " << nThreads << " threads for the FFT." << std::endl;
    ...
    for (auto ii = kgrid.begin(); ii != kgrid.end(); ++ii) {
        ...
        *ii *= W[std::abs(kx)] * W[std::abs(ky)] * W[kz]; // Correction for mass assignment
        ...
        int i;
        if (bLog) {
            i = int(log(k) / log(iNyquist) * nBins);
        } else {
            i = int(k / iNyquist * nBins);
        }
        if (i > 0 && i < nBins) {
            ak(i) += k;
            pk(i) += std::norm(*ii);
