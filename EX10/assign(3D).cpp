...
int main(int argc, char* argv[])
{
    ...
    //                     &local_n0,
    //                     &local_0_start);
    //Exercise2  Add new variables for transposed info:
    ptrdiff_t local_n0, local_0_start;
    ptrdiff_t local_n1, local_1_start;   // <-- new from fftwf_mpi_local_size_3d_transposed
    ptrdiff_t complex_count;
    complex_count = fftwf_mpi_local_size_3d_transposed(
        nGrid, nGrid, nGrid,           // the full real domain size
        MPI_COMM_WORLD,
        &local_n0, &local_0_start,     // regular data distribution
        &local_n1, &local_1_start);    // transposed data distribution
    ...
    if (irank == 0) {
        std::cerr << "Rank=" << irank
                  ...
                  << "         local_n1=" << local_n1
                  << " local_1_start=" << local_1_start << std::endl;
    }
    
    if (local_n0 == local_n1 && local_0_start == local_1_start) {
        std::cerr << "Rank=" << irank << ": local_n0 == local_n1 and local_0_start == local_1_start.\n";
    } else {
        std::cerr << "Rank=" << irank << ": They differ!\n";
    }
    // create slab + ghost region
...
#endif
    //Exercise1 Added | FFTW_MPI_TRANSPOSED_OUT to skip final transpose
    auto plan = fftwf_mpi_plan_dft_r2c_3d(
        nGrid, nGrid, nGrid,
        slab.dataFirst(),
        reinterpret_cast<fftwf_complex*>(kslabTransposed_noghost.dataFirst()),
        MPI_COMM_WORLD,
