...
int main(int argc, char *argv[]) {
    ...
    //Exercise 4
    int nbins = 80;
    float kmax = std::sqrt( (nGrid/2.0f)*(nGrid/2.0f)*3.0f );

    // 3) Prepare vectors to accumulate partial sums + counts
    std::vector<float> fPowerLog(nbins, 0.0f);
    std::vector<int>   nPowerLog(nbins, 0);

    // 4) Loop over k-space
    for (int ix = 0; ix < nGrid; ix++) {
        // Convert index -> wave number in x
        ...
        for (int iy = 0; iy < nGrid; iy++) {
            ...
            for (int iz = 0; iz < k_nz; iz++) {
                ...
                // Skip the DC mode (k=0 => log undefined)
                if (k_abs <= 0.0f) {
                    continue;
                }

                // 5) Compute log bin index from eqn(4) in your PDF
                //    iBin = floor( log(k_abs)/log(kmax) * nbins ).
                //    Make sure we stay in [0, nbins-1]
                float ratio = std::log(k_abs) / std::log(kmax); 
                int iBin = int(std::floor(ratio * nbins));
    ...
    // 6) Output the result as a function of "bin center" in log space
    //    We'll write to "power_log.dat"
    std::ofstream outfile("power_log.dat");
    outfile << "# iBin   k_center   P(k)\n";

    // The width in log space is log(kmax)/nbins
    float log_delta = std::log(kmax) / float(nbins);

    for (int i = 0; i < nbins; i++) {
        // The log range for bin i is [i*log_delta, (i+1)*log_delta]
        // The bin center is the geometric mean: exp( (log_min + log_max)/2 ) 
        // or simpler: exp( (i + 0.5)*log_delta ), since log_min = i*log_delta
        float log_min = i*log_delta;
        float log_max = (i+1)*log_delta;
        float k_center = std::exp(0.5f*(log_min + log_max));

        // If you want an alternative, per eqn(4) exactly:
        //   k_center = exp( (float(i)+0.5f)/nbins * log(kmax) );  // same result

        if (nPowerLog[i] > 0) {
            float avgPower = fPowerLog[i] / float(nPowerLog[i]);
            outfile << i << "  " << k_center << "  " << avgPower << "\n";
        } else {
            outfile << i << "  " << k_center << "  0.0\n";
        }
    }
    outfile.close();
    std::cerr << "Done writing power_log.dat\n";
}
